...
public interface ItemDisplayRepo extends JpaRepository<ItemDisplay, Integer> {
	...
	@Query(value = "SELECT " + 
			...
			"                        t2.item_id = t7.item_id " + 
			"                    ORDER BY " + 
			"                        t1.sp_rate_amt, t2.item_sort_id", nativeQuery = true)
	public List<ItemDisplay> getAllItemByFrPriceLowToHigh(@Param("frId") int frId,@Param("type") int type,@Param("applicableFor") int applicableFor);
	...
	//SORT PRICE LOW TO HIGH
		...
		@Query(value = "SELECT " + 
				...
				"                        t2.item_id = t7.item_id " + 
				"                    ORDER BY " + 
				"                        t1.sp_rate_amt DESC, t2.item_sort_id", nativeQuery = true)
		public List<ItemDisplay> getAllItemByFrPriceHighToLow(@Param("frId") int frId,@Param("type") int type,@Param("applicableFor") int applicableFor);
		...
		//SORT PRICE LOW TO HIGH
			...
			@Query(value = "SELECT " + 
					...
					"                        t2.item_id = t7.item_id " + 
					"                    ORDER BY " + 
					"                        t2.rating DESC, t2.item_sort_id", nativeQuery = true)
			public List<ItemDisplay> getAllItemByFrRatingHighToLow(@Param("frId") int frId,@Param("type") int type,@Param("applicableFor") int applicableFor);
			...
			@Query(value = "SELECT " + 
					...
					"    m_item_sup sup " + 
					"WHERE " + 
					"    i.del_status = 0 AND d.del_status = 0 AND d.is_used = 0 AND i.id = d.item_id AND i.item_grp1 = c.cat_id AND i.item_grp2 = s.sub_cat_id AND c.del_status = 0 AND s.del_status = 0 AND i.id = sup.item_id AND sup.del_status = 0 AND d.ex_float1 IN(:ratings)" + 
					") t2 " + 
					"ON " + 
					"    t1.item_id = t2.item_id " + 
					"LEFT JOIN( " + 
					...
					"                    ORDER BY " + 
					"                        t2.rating DESC, t2.item_sort_id", nativeQuery = true)
			public List<ItemDisplay> getAllItemByFrByRating(@Param("frId") int frId,@Param("type") int type,@Param("applicableFor") int applicableFor,@Param("ratings") List<String> ratings);
